...
(define (every lam lst) (if (null? lst) nil (cons (lam (car lst)) (every lam (cdr lst)))))
; expect every

(every (lambda (x) (+ x 3)) '(1 2 3 4 5))
; expect (4 5 6 7 8)

(define (dog x y z) (let ((x 3) (y 5)) (+ (* z y) x)))
; expect dog

(dog 1 1 1)
;expect 8

(define (cow cow chicken) (chicken))
;expect cow

(cow cow cow)
; expect Error

(eq? (let ((x 2) (y 3)) (+ x y)) ((lambda (x y) (+ x y)) 2 3))
;expect #t

(define mu-func (mu () (+ c d)))
; expect mu-func

(define (helper x y) (define mu-function (mu () (+ x y))) (let ((x 7)(y 8)) (mu-function)));
;expect helper
(helper 9 10)
; expect 15
...
(define (func x y)
  (begin
    (define (newFunc x y)
      (if (= x 0)
        y
        (newFunc (- x 1) (cons x y))))
    (newFunc x y))
)
; expect func

(map (lambda (element) (+ 1 element)) (list 1 2 3))
; expect (2 3 4)
...
(and (* x x (+ (/ x 4) 10)) #f (* 10 (/ 16 x)))
; expect #f

(eq? (let ((x 10) (y 4) (z 3)) (* x (- y z))) ((lambda (x y z) (* x (- y z))) 10 4 3))
; expect #t
