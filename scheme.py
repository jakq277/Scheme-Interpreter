...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        combiner = scheme_eval(first,env)
        if isinstance(combiner,MacroProcedure):
            operands = combiner.apply_macro(rest,env)
            return scheme_eval(operands,env)
        else:
            return scheme_apply(combiner, rest.map(lambda x: scheme_eval(x,env)),env)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if not (formals is nil and vals is nil):
            raise SchemeError("number of vals not number of formals")
        return child_frame
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        new_expr = Pair(expressions.first.second,expressions.second)
        env.define(target.first,do_lambda_form(new_expr ,env))
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    expr, symbols = nil, nil
    while bindings is not nil:
        val = bindings.first
        check_form(val, 2, 2)
        expr = Pair(scheme_eval(val.second.first,env),expr)
        symbols = Pair(val.first,symbols)
        bindings = bindings.second
    check_formals(symbols)
    return env.make_child_frame(symbols,expr)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if isinstance(expressions.first,Pair) and scheme_symbolp(expressions.first.first):
        ...
        new_proc = MacroProcedure(expressions.first.second, expressions.second,env)
        env.define(expressions.first.first,new_proc)
        return expressions.first.first
    else:
        raise SchemeError("Not a valid macro")
    # END Problem 21
